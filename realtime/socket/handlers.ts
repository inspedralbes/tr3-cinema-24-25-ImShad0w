...
async function releaseSeats(io: Server, socket: Socket, user: any) {
  if (user.reservedSeats.length > 0) {
    ...
    try {
      const response = await fetch("http://passmaster_backend:8000/api/seats/release", {
        method: "POST",
...
export function setupSocketHandlers(io: Server, socket: Socket): void {
  ...
  socket.on("enterEvent", async (data: any) => {
    ...
    try {
      ...
      const response = await fetch(`http://passmaster_backend:8000/api/event/${eventId}`);
  ...
  socket.on("reserveSeats", async (data: any) => {
    ...
    try {
      const response = await fetch("http://passmaster_backend:8000/api/seats/reserve", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          seat_ids: seatIds,
  ...
  socket.on("buySeats", async (data: any) => {
    ...
    try {
      const response = await fetch("http://passmaster_backend:8000/api/seats/buy", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          seat_ids: seatIds,
